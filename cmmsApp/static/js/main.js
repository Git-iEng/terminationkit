/* ==========================================================
   landing-page-termination-kit.js
...
/* ===== CONFIG ===== */
const SOLAR = {
  revealSelector: '.reveal-termination-kit',
  inViewClass: 'in-view-termination-kit',
...
(function initLogosPager() {
  const wrap = document.querySelector('.logos-wrap-termination-kit');
  const track = document.getElementById('logos-track-termination-kit');
  const dotsWrap = document.getElementById('dots-termination-kit');
  if (!wrap || !track || !dotsWrap) return;

  const dots = Array.from(dotsWrap.querySelectorAll('.dot-termination-kit'));
  const RESUME_DELAY = 3500; // ms after click before continuous scroll resumes
  let resumeTimer = null;

  // set active dot helper
  function setActiveDot(idx) {
    dots.forEach((d, i) => d.classList.toggle('is-active-termination-kit', i === idx));
  }
  setActiveDot(0);
  ...
  function enterManualMode() {
    track.classList.add('manual-termination-kit');
  ...
  function resumeContinuous() {
    ...
    track.classList.remove('manual-termination-kit');
  ...
  const ro = new ResizeObserver(() => {
    const active = dots.findIndex(el => el.classList.contains('is-active-termination-kit'));
    if (active > -1 && track.classList.contains('manual-termination-kit')) {
  ...
  wrap.addEventListener('mouseenter', () => {
    if (!track.classList.contains('manual-termination-kit')) {
      track.style.animationPlayState = 'paused';
    }
  });
...
(function initImpactCounters() {
  const items = document.querySelectorAll('.stat-value-termination-kit-impact');
...
(function initSolutionsToggle() {
  const grid = document.getElementById('solutions-grid-termination-kit-solution');
  const btn = document.getElementById('solutions-toggle-btn-termination-kit-solution');
  if (!grid || !btn) return;

  function setState(expanded) {
    grid.classList.toggle('is-collapsed-termination-kit-solution', !expanded);
...
(function initProjectsCarousel() {
  const viewport = document.getElementById('projects-viewport-termination-kit-projects');
  const prevBtn = document.querySelector('.prev-termination-kit-projects');
  const nextBtn = document.querySelector('.next-termination-kit-projects');
...
(function initSolarTypes() {
  const tabs = Array.from(document.querySelectorAll('.tab-btn-termination-kit-types'));
  const panels = {
    'on-grid': document.getElementById('panel-on-grid-termination-kit-types'),
    'off-grid': document.getElementById('panel-off-grid-termination-kit-types'),
    'hybrid': document.getElementById('panel-hybrid-termination-kit-types')
  };
  ...
  function activate(type) {
    ...
    tabs.forEach(btn => {
      ...
      btn.classList.toggle('is-active-termination-kit-types', isActive);
    ...
    Object.entries(panels).forEach(([key, el]) => {
      const show = key === type;
      if (!el) return;
      el.classList.toggle('is-active-termination-kit-types', show);
      el.hidden = !show;
      if (show) {
  ...
  document.querySelector('.tabs-termination-kit-types')?.addEventListener('keydown', (e) => {
    const idx = tabs.findIndex(b => b.classList.contains('is-active-termination-kit-types'));
...
/* ==========================================================
   Scoped tabs for all .section-types-termination-kit-types
   (no global getElementById; supports multiple instances)
   ========================================================== */
(function initAllSolarTypeTabs() {
  document.querySelectorAll('.section-types-termination-kit-types').forEach(section => {
    const tabsWrap = section.querySelector('.tabs-termination-kit-types');
    if (!tabsWrap) return;

    const tabs = Array.from(section.querySelectorAll('.tab-btn-termination-kit-types'));
    const panels = Array.from(section.querySelectorAll('.panel-termination-kit-types'));
    ...
    function activate(btn) {
      ...
      tabs.forEach(t => {
        ...
        t.classList.toggle('is-active-termination-kit-types', isActive);
      ...
      panels.forEach(p => {
        ...
        p.hidden = !show;
        p.classList.toggle('is-active-termination-kit-types', show);
    ...
    tabsWrap.addEventListener('keydown', (e) => {
      ...
      e.preventDefault();
      const current = tabs.findIndex(t => t.classList.contains('is-active-termination-kit-types'));
      const dir = e.key === 'ArrowRight' ? 1 : -1;
    ...
    activate(tabs.find(t => t.classList.contains('is-active-termination-kit-types')) || tabs[0]);
  });
})();
...
(function () {
  const grid = document.getElementById('grid-termination-kit-card-with-animation');
  if (!grid) return;
  const cards = grid.querySelectorAll('.card-termination-kit-card-with-animation');

  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        e.target.classList.add('show-termination-kit-card-with-animation');
      } else {
        // remove so it replays when scrolling back (LIFO feel)
        e.target.classList.remove('show-termination-kit-card-with-animation');
      }
    });
  }, { threshold: 0.18 });
...
(() => {
  const targets = document.querySelectorAll(
    '.reveal-top-termination-kit1-why-us, .reveal-up-termination-kit1-why-us, .reveal-right-termination-kit1-why-us, .reveal-left-termination-kit1-why-us'
  );

  const io = new IntersectionObserver((entries) => {
    entries.forEach((e) => {
      if (e.isIntersecting) {
        e.target.classList.add('reveal-in-termination-kit1-why-us');
...
(() => {
  const targets = document.querySelectorAll(
    '.reveal-top-termination-kit-csc-products, .reveal-left-termination-kit-csc-products, .reveal-right-termination-kit-csc-products, .reveal-up-termination-kit-csc-products'
  );
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting){
        e.target.classList.add('reveal-in-termination-kit-csc-products');
        io.unobserve(e.target);
      }
    });
  }, { threshold: 0.18 });
...
(() => {
  ...
  const close = modal.querySelector('.modal-close-csc-termination-kit-csc-products');
  ...
  document.querySelectorAll('.view-doc-csc-termination-kit-csc-products').forEach(a => {
  ...
  document.querySelectorAll('.request-download-csc-termination-kit-csc-products').forEach(btn => {
    btn.addEventListener('click', () => {
  ...
  modal.querySelector('.modal-backdrop-csc-termination-kit-csc-products')
